...
class ControllerCatalogDownload extends Controller {  
	private $error = array();
  	...
  	public function index() {
		$this->load->idioma('catalog/download');
  	...
  	private function getForm() {
    	...
    	$this->data['entry_filename'] = $this->idioma->get('entry_filename');
    	$this->data['entry_remaining'] = $this->idioma->get('entry_remaining');
    	$this->data['entry_update'] = $this->idioma->get('entry_update');
  	...
  	private function validateForm() { 
		...
		if ($this->request->files['download']['name']) {
			if ((strlen(utf8_decode($this->request->files['download']['name'])) < 3) || (strlen(utf8_decode($this->request->files['download']['name'])) > 128)) {
        		$this->error['download'] = $this->idioma->get('error_filename');
	  		}	  	
			
			if (substr(strrchr($this->request->files['download']['name'], '.'), 1) == 'php') {
       	   		$this->error['download'] = $this->idioma->get('error_filetype');
       		}	
						
			if ($this->request->files['download']['error'] != UPLOAD_ERR_OK) {
				$this->error['warning'] = $this->idioma->get('error_upload_' . $this->request->files['download']['error']);
			}
		}
  	...
  	private function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $download_id) {
  			$product_total = $this->model_catalog_evento->getTotalProductsByDownloadId($download_id);
