...
class ControllerCommonDashboard extends Controller {   
	public function index() {
		...
		$this->document->setTitle('Indicadores');
		
    	$this->data['heading_title'] = 'Indicadores';
		
		$this->data['text_overview'] = 'General';
		...
		switch ($range) {
			...
			case 'month':
				for ($i = 1; $i <= date('t'); $i++) {
					...
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "solicitud` WHERE order_status_id > '0' AND (DATE(clientes_fdc) = '" . $this->db->escape($date) . "') GROUP BY DAY(clientes_fdc)");
			...
			case 'year':
				for ($i = 1; $i <= 12; $i++) {
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "solicitud` WHERE order_status_id > '0' AND YEAR(clientes_fdc) = '" . date('Y') . "' AND MONTH(clientes_fdc) = '" . $i . "' GROUP BY MONTH(clientes_fdc)");
