	...
	*/
	...
	public function export() {
    	...
    	foreach ($results as $result) {
			...
			if ($this->request->get['eventos_id'] == 114) {
...
//Nombre de la Mascota			
			...
			} else if ($this->request->get['eventos_id'] == 107 || $this->request->get['eventos_id'] == 115) {
...
//			}
	...
	public function info() {
		...
		if ($participante_info) {
			...
			$this->data['text_notify'] = $this->idioma->get('text_notify');
	...
	*/
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$fecha_evento = $this->model_catalog_evento->getEventoFecha($id_evento);
			$status_evento = $this->model_catalog_evento->getEventoStatus($id_evento);
			$hoy = date("Y-m-d");
...
//		$this->url->link('sale/participantes/info', 'token=' . $this->session->data['token'] . '&eventos_participantes_id=' . $result['eventos_participantes_id'] . $url, 'SSL');
  	...
  	private function validateForm() { 
		...
		if ((strlen(utf8_decode($this->request->post['eventos_participantes_fdn'])) != 10)) {
			...
			if (!preg_match("/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/", $this->request->post['eventos_participantes_fdn'])) {	
