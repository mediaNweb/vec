                        ...
                        <div id="dato-cupos">
                            <tr>
                                <td>
                                    ...
                                    <b>Notificar al Participante:</b><br />
                                </td>
                                <td>
                                    <input type="checkbox" name="notify" value="true" checked="checked" />                                </td>
                            </tr>
                        </div>
                    </div>
                    <div class="dato">
    ...
    function busca_categoria() {
		<?php if ($eventos_id != 96) { ?>
			...
			$.ajax({
				...
				data: 'eventos_id=' + <?php echo $eventos_id; ?> + '&edad=' + edad_part + '&sexo=' + sexo_part + '&clientes_id=' + cedula_part,
				//doc.select.options[doc.select.selectedIndex].text;
				contentType: "application/json; charset=utf-8",
				dataType: 'json',
				beforeSend: function() {
        ...
        $.ajax({
            ...
            success: function(json) {
                ...
                if (json['output']) {
					...
					$.ajax({
						url: "index.php?route=catalog/transcripcion/estado&token=<?php echo $token; ?>&paises_id=" + paises_id + "&estados_id=" + estados_id,
						type: 'post',
						dataType: 'json',
						success: function (json) {
					...
					document.getElementById('paises_id').selectedIndex = 0;
					document.getElementById('estados_id').selectedIndex = 0;
                    $("#clientes_genero").removeAttr('disabled');
                    $("#clientes_fdn").val('');
                    $("#clientes_fdn").removeAttr('readonly');
                    $("#clientes_edad").val('');
