      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        ...
        <table id="zone-to-geo-zone" class="list">
          ...
          <tbody id="zone-to-geo-zone-row<?php echo $zone_to_geo_zone_row; ?>">
            <tr>
              <td class="left"><select name="zone_to_geo_zone[<?php echo $zone_to_geo_zone_row; ?>][paises_id]" id="country<?php echo $zone_to_geo_zone_row; ?>" onchange="$('#zone<?php echo $zone_to_geo_zone_row; ?>').load('index.php?route=localidad/geo_zone/zone&token=<?php echo $token; ?>&paises_id=' + this.value + '&zone_id=0');">
                  <?php foreach ($paises as $country) { ?>
                  <?php  if ($country['paises_id'] == $zone_to_geo_zone['paises_id']) { ?>
...
$('#zone-id').load('index.php?route=localidad/geo_zone/zone&token=<?php echo $token; ?>&paises_id=' + $('#country-id').attr('value') + '&zone_id=0');
//--></script>
<?php $zone_to_geo_zone_row = 0; ?>
<?php foreach ($zone_to_geo_zones as $zone_to_geo_zone) { ?>
<script type="text/javascript"><!--
$('#zone<?php echo $zone_to_geo_zone_row; ?>').load('index.php?route=localidad/geo_zone/zone&token=<?php echo $token; ?>&paises_id=<?php echo $zone_to_geo_zone['paises_id']; ?>&zone_id=<?php echo $zone_to_geo_zone['zone_id']; ?>');
//--></script>
<?php $zone_to_geo_zone_row++; ?>
<?php } ?>
<script type="text/javascript"><!--
...
function addGeoZone() {
	...
	html += '<tr>';
	html += '<td class="left"><select name="zone_to_geo_zone[' + zone_to_geo_zone_row + '][paises_id]" id="country' + zone_to_geo_zone_row + '" onchange="$(\'#zone' + zone_to_geo_zone_row + '\').load(\'index.php?route=localidad/geo_zone/zone&token=<?php echo $token; ?>&paises_id=\' + this.value + \'&zone_id=0\');">';
	<?php foreach ($paises as $country) { ?>
	...
	$('#zone' + zone_to_geo_zone_row).load('index.php?route=localidad/geo_zone/zone&token=<?php echo $token; ?>&paises_id=' + $('#country' + zone_to_geo_zone_row).attr('value') + '&zone_id=0');
