...
4 = Equipo
5 = Posicion General
6 = Posicion por Genero
7 = Posicion por Categoria
8 = Vueltas
9 = Tiempo Oficial
10 = Tiempo Tag
*/
...
//	        $handle = fopen($_FILES['prev_times']['tmp_name'], 'r');
	...
	//			if((preg_match("/^([0-9]+)$/", $data_csv[0])) and (preg_match("/^(0?\d|1[0-2]):([0-5]\d):([0-5]\d)$/", $data_csv[1]))){
				if(preg_match("/^([0-9]+)$/", $data_csv[0])) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "resultados SET id_evento = '" . (int)$eventos_id . "', cedula = '" . $data_csv[0] . "', nombre = '" . $data_csv[1] . "', numero = '" . $data_csv[2] . "', categoria = '" . $data_csv[3] . "', equipo = '" . $data_csv[4] . "', pos_general = '" . $data_csv[5] . "', pos_genero = '" . $data_csv[6] . "', pos_categoria = '" . $data_csv[7] . "', vueltas = '" . $data_csv[8] . "', time_tag = '" . $data_csv[9] . "', time_oficial = '" . $data_csv[10] . "'");
				}
			}
	...
	//			if((preg_match("/^([0-9]+)$/", $data_csv[0])) and (preg_match("/^(0?\d|1[0-2]):([0-5]\d):([0-5]\d)$/", $data_csv[1]))){
				if(preg_match("/^([0-9]+)$/", $data_csv[0])) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "resultados SET id_evento = '" . (int)$eventos_id . "', cedula = '" . $data_csv[0] . "', nombre = '" . $data_csv[1] . "', numero = '" . $data_csv[2] . "', categoria = '" . $data_csv[3] . "', pos_general = '" . $data_csv[5] . "', pos_genero = '" . $data_csv[6] . "', pos_categoria = '" . $data_csv[7] . "', time_tag = '" . $data_csv[9] . "', time_oficial = '" . $data_csv[10] . "'");
				}
			}
...
//             $query = $this->db->query("SELECT eventos_descripcion_numeracion_id_tipo AS tipo FROM " . DB_PREFIX . "eventos_descripcion ed WHERE ed.eventos_descripcion_id_evento = '" . (int)$eventos_id . "'");
	...
	public function getEventoInscripciones($eventos_id) {
		$query = $this->db->query("SELECT eventos_inscripciones AS inscripciones FROM " . DB_PREFIX . "eventos e WHERE e.eventos_id = '" . (int)$eventos_id . "'");

		return $query->row['inscripciones'];
	}
