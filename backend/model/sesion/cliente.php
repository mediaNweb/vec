<?php
    class ModelSesionCliente extends Model {
		...
		public function isParticipante($clientes_id) {
	
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "eventos_participantes WHERE eventos_participantes_id_cliente = '" . $clientes_id . "' ORDER BY `eventos_participantes_id` DESC LIMIT 1");
        ...
        public function getParticipante($clientes_id) {
            $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "eventos_participantes WHERE eventos_participantes_id_cliente = '" . $clientes_id . "' ORDER BY `eventos_participantes_id` DESC LIMIT 1");

            if ($query->num_rows) {
				$edad = $this->getEdad($query->row['eventos_participantes_fdn']);
                $datos = array(
                'nombre'            => $query->row['eventos_participantes_nombres'],
                'apellido'          => $query->row['eventos_participantes_apellidos'],
                'cedula'            => $query->row['eventos_participantes_cedula'],
                'nacimiento'        => $query->row['eventos_participantes_fdn'],
				'edad'				=> $edad,
                'nacionalidad'      => '',
                'sexo'              => $query->row['eventos_participantes_genero'],
                'sangre'            => '',
                'direccion'         => '',
                'pais'              => $query->row['eventos_participantes_id_pais'],
                'estado'            => $query->row['eventos_participantes_id_estado'],
                'tlf_habitacion'	=> '',
                'celular'			=> $query->row['eventos_participantes_cel'],
                'mail'				=> $query->row['eventos_participantes_email'],
                'twitter'           => '',
                );
                return $datos;
            } else {
...
//					'categoria_' . $key	=> $value,
        ...
        public function getCategoriaCircuito($eventos_id, $edad, $genero, $grupo) {
			$categoria_data = array();
			
            $categoria_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "eventos_categorias WHERE eventos_categorias_id_evento = '" . (int)$eventos_id . "' AND eventos_categorias_edad_desde <= " . $edad . " AND eventos_categorias_edad_hasta >= " . $edad . " AND eventos_categorias_grupo = '" . $grupo . "' AND (eventos_categorias_genero = 'Ambos' OR eventos_categorias_genero = '" . $genero . "') AND eventos_categorias_tipo <> 'Opcional'");
...
//					'categoria_' . $key	=> $value,
        ...
        public function getHistorialCircuito($cedula, $eventos_id) {
            $query = $this->db->query("SELECT eventos_participantes_grupo AS grupo FROM " . DB_PREFIX . "eventos_participantes ep WHERE ep.eventos_participantes_cedula = '" . $cedula . "' AND ep.eventos_participantes_id_evento = '" . (int)$eventos_id . "' LIMIT 1");
