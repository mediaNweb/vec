<?php
class ControllerEventoBib extends Controller
{
	private $error = array();
	...
	public function index()
	{
		...
		if ($resultado) {
			...
			$this->data['ritmo'] = ($ritmo != '') ? date($time_format, strtotime($ritmo)) : 'n/a';
			...
			if ($evento_info) {
				...
				if (!empty($evento_info['eventos_imagen_afiche']) && (file_exists(DIR_IMAGE . $evento_info['eventos_imagen_afiche']))) {
					$img_bib = $this->model_tool_image->resize($evento_info['eventos_imagen_afiche'], $setting['width'], $setting['height']);
					$design = 1;
				} else {
					$img_bib = $this->model_tool_image->resize('racetimer_certificate_mockup.jpg', $setting['width'], $setting['height']);
					$design = 0;
				}

				$mpdf->SetWatermarkImage($img_bib);
				...
				$this->data['text_bib']							= $this->language->get('text_bib');
			}
			...
			if ($eventos_certificado_foto != 0) {
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/evento/bib.php')) {
					$this->template = $this->config->get('config_template') . '/evento/bib.php';
				} else {
					$this->template = 'evento/bib.php';
				}
			}
